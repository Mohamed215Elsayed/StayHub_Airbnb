...
export class VerifyOtpUseCase {
    ...
    private validateOtpBeforeVerify(otp: OtpRawResponseDto | null, body: VerifyOtpDto) {
        if (!otp) throw new CustomBadRequestException('otp.OTP_NOT_FOUND');
        if (otp.code !== body.code) throw new CustomBadRequestException('otp.INVALID_OTP');
        if (new Date() > otp.expiresAt)
            throw new CustomBadRequestException('otp.OTP_EXPIRED');
        if (otp.isVerified) throw new CustomBadRequestException('otp.OTP_ALREADY_VERIFIED');
    }
}